...
import {fetchStorage, deleteStorage,saveStorage} from '../../helper/localStorage'
import './Shopping.css'
const Shopping = () => {
    ...
    <ul className='list'>
      {shopping.map((el)=>{
        return (
          ...
          className='li'
        >
          <div className='li-info-wrapper' onClick={strikethroughText}>
            <img
              src={el.image}
              alt={el.name}
            />{' '}
          ...
          <button className='delete' id={el.id} onClick={deleteItem}>❌</button>
        </li>
        )
      })}
    </ul>
